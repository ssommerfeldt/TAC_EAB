...
Public Class AddEditOrderProduct
    ...
    Private Sub AddEditOrderProduct_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        _OrderType = GetField("ORDERTYPE", "SALESORDER", " SALESORDERID='" & _SalesOrderId & "'", Me.SlxApplication.ConnectionString)
        _AccountId = GetField("Accountid", "Salesorder", "Salesorderid ='" & _SalesOrderId & "'", Me.SlxApplication.ConnectionString)
        _CurrencyId = GetField("Isnull(CURRENCYCODE,'CAD')", "Account", "AccountID = '" & _AccountId & "'", Me.SlxApplication.ConnectionString)

        _CustomerId = GetField("CustomerId", "AccountFinancial", "AccountId = '" & _AccountId & "'", Me.SlxApplication.ConnectionString)
        _CompanyCode = GetField("CompanyCode", "AccountFinancial", "AccountId = '" & _AccountId & "'", Me.SlxApplication.ConnectionString)
        _CustKey = GetField("CustKey", "tarCustomer", "CustID = '" & _CustomerId & "' And CompanyID = '" & _CompanyCode & "'", Me.SlxApplication.ConnectionString)

        _AccountManagerId = GetField("ACCOUNTMANAGERID", "ACCOUNT", "ACCOUNTID='" & _AccountId & "'", Me.SlxApplication.ConnectionString)
        _CompanyId = GetField("MASCOMPANYID", "USERSECURITY", "USERID='" & _AccountManagerId & "'", Me.SlxApplication.ConnectionString)
        ...
        useTemplateAccountPricing = GetField("Isnull(USEPRICINGTEMPLATEACCOUNT,'F')", "ACCOUNT", "ACCOUNTID='" & _AccountId & "'", Me.SlxApplication.ConnectionString)
        TemplateAccountId = GetField("PRICINGTEMPLATEACCOUNTID", "ACCOUNT", "ACCOUNTID='" & _AccountId & "'", Me.SlxApplication.ConnectionString)
    ...
    Private Sub txtSKU_Leave(sender As Object, e As EventArgs) Handles txtSKU.Leave
        ...
        If _OrderType = "Sales Order" Then
            ...
            _ProductId = GetField("PRODUCTID", "PRODUCT", _filter, Me.SlxApplication.ConnectionString)
    ...
    Sub CalcExtendedPrice()
        ...
        Dim Price
        Dim ExtendedPrice
        Quantity = CDbl(CheckEmpty(txtQuantity.Text, 0))
        Price = RoundUp(CheckEmpty(txtPrice.Text, 0), 2) 'Value'(Price - (Price * Discount))
        ExtendedPrice = RoundUp(Price * Quantity, 2)
        txtExtendedPrice.Text = ExtendedPrice
    End Sub
    ...
    '    strWareHouseId = ExecuteScalarQuery(strSQL, Me.SlxApplication.ConnectionString)
    ...
    Function GetWareHouseId(ByVal SalesOrderId As String) As String
        ...
        Try
            objConn.Open(Me.SlxApplication.ConnectionString)
            With objRS
    ...
    Sub GetProductInfo(ByVal Productid As String)
        ...
        _SalesOrderItemId = GetField("SALESORDERITEMSID", "SALESORDERITEMS", " SALESORDERID='" & _SalesOrderId & "' AND PRODUCTID ='" & _ProductId & "'", Me.SlxApplication.ConnectionString)

        ' ==============================================================
        ' Load the common product information
        ' ==============================================================
        'lueProduct.LookupID = Productid
        txtDescription.Text = GetField("DESCRIPTION", "PRODUCT", "PRODUCTID='" & Productid & "'", Me.SlxApplication.ConnectionString)
        'lueProduct.Text = GetField("NAME", "PRODUCT", "PRODUCTID='" & Productid & "'")
        txtUPC.Text = GetField("Isnull(UPC,'')", "PRODUCT", "PRODUCTID='" & Productid & "'", Me.SlxApplication.ConnectionString)
        txtSKU.Text = GetField("Isnull(ACTUALID,'')", "PRODUCT", "PRODUCTID='" & Productid & "'", Me.SlxApplication.ConnectionString)

        txtStatus.Text = GetField("STATUS", "PRODUCT", "PRODUCTID='" & Productid & "'", Me.SlxApplication.ConnectionString)
        ...
        Else
            ...
            If _OrderType <> "Return Order" Then
                ...
                stockCardId = GetField("IsNull(STOCKCARDITEMSID, '')", "STOCKCARDITEMS", "ACCOUNTID = '" & _AccountId & "' AND PRODUCTID = '" & Productid & "'", Me.SlxApplication.ConnectionString)
                If stockCardId <> "" Then

                    ListPrice = RoundUp(CDbl(GetField("IsNull(LISTPRICE, 0)", "STOCKCARDITEMS", "ACCOUNTID = '" & _AccountId & "' AND PRODUCTID = '" & Productid & "'", Me.SlxApplication.ConnectionString)), 2)
                    Discount = GetField("IsNull(MARGIN, 0)", "STOCKCARDITEMS", "ACCOUNTID = '" & _AccountId & "' AND PRODUCTID = '" & Productid & "'", Me.SlxApplication.ConnectionString)
                ...
                Else
                    ...
                    stockCardId = GetField("IsNull(s.STOCKCARDITEMSID, '')",
                        ...
                        "s.ACCOUNTID = '" & _AccountId & "' AND p.ACTUALID = '" & txtSKU.Text & "'", SlxApplication.ConnectionString)

                    If stockCardId <> "" Then
                        ListPrice = RoundUp(CDbl(GetField("IsNull(LISTPRICE, 0)", "STOCKCARDITEMS", "ACCOUNTID = '" & _AccountId & "' AND STOCKCARDITEMSID = '" & stockCardId & "'", Me.SlxApplication.ConnectionString)), 2)
                        Discount = GetField("IsNull(MARGIN, 0)", "STOCKCARDITEMS", "ACCOUNTID = '" & _AccountId & "' AND STOCKCARDITEMSID = '" & stockCardId & "'", Me.SlxApplication.ConnectionString)
                    ...
                    Else
                        ...
                        ItemKey = GetField("MASItemKey", "Product", "ProductId = '" & Productid & "'", Me.SlxApplication.ConnectionString)
                        ...
                        If ListPrice = 0 Then
                            ...
                            ItemKey = GetField("MASItemKey", "Product", "actualid = '" & txtSKU.Text & "' and warehouseid ='DEL'", Me.SlxApplication.ConnectionString)
                            Productid = GetField("Productid", "Product", "actualid = '" & txtSKU.Text & "' and warehouseid ='DEL'", Me.SlxApplication.ConnectionString) 'Set ProductId as it isn't on the StockCard
                            ListPrice = RoundUp(GetListPrice(ItemKey, _CurrencyId, _CustKey), 2)
                        End If
            ...
            ElseIf _OrderType = "Return Order" Then
                ...
                ' Check for the item in the stock card
                '===============================================================
                stockCardId = GetField("IsNull(STOCKCARDITEMSID, '')", "STOCKCARDITEMS", "ACCOUNTID = '" & _AccountId & "' AND PRODUCTID = '" & StockCardProductId & "'", SlxApplication.ConnectionString)
                If stockCardId <> "" Then

                    ListPrice = RoundUp(CDbl(GetField("IsNull(LISTPRICE, 0)", "STOCKCARDITEMS", "ACCOUNTID = '" & _AccountId & "' AND PRODUCTID = '" & StockCardProductId & "'", SlxApplication.ConnectionString)), 2)
                    Discount = GetField("IsNull(MARGIN, 0)", "STOCKCARDITEMS", "ACCOUNTID = '" & _AccountId & "' AND PRODUCTID = '" & StockCardProductId & "'", SlxApplication.ConnectionString)
                ...
                Else
                    ...
                    stockCardId = GetField("IsNull(s.STOCKCARDITEMSID, '')",
                        "STOCKCARDITEMS s Inner Join PRODUCT p On p.PRODUCTID = s.PRODUCTID",
                        "s.ACCOUNTID = '" & _AccountId & "' AND p.ACTUALID = '" & txtSKU.Text & "'", Me.SlxApplication.ConnectionString)

                    If stockCardId <> "" Then
                        ListPrice = RoundUp(CDbl(GetField("IsNull(LISTPRICE, 0)", "STOCKCARDITEMS", "ACCOUNTID = '" & _AccountId & "' AND STOCKCARDITEMSID = '" & stockCardId & "'", SlxApplication.ConnectionString)), 2)
                        Discount = GetField("IsNull(MARGIN, 0)", "STOCKCARDITEMS", "ACCOUNTID = '" & _AccountId & "' AND STOCKCARDITEMSID = '" & stockCardId & "'", SlxApplication.ConnectionString)
    ...
    Function GetOriginalProductPricing(ByVal ActualId As String,
        ...
        If _OrderType = "Return Order" Then
            ...
            IsInternalAccount = GetField("IsNull(InternalAccount,'')", "Account", "ACCOUNTID = '" & _AccountId & "'", SlxApplication.ConnectionString)
            ...
            '===============================================================
            Dim stockCardId
            stockCardId = GetField("IsNull(STOCKCARDITEMSID, '')", "STOCKCARDITEMS", "ACCOUNTID = '" & AccountId & "' AND PRODUCTID = '" & OriginalProductId & "'", SlxApplication.ConnectionString)
            If stockCardId <> "" Then

                Margin = CDbl(GetField("IsNull(MARGIN, 0)", "STOCKCARDITEMS", "ACCOUNTID = '" & AccountId & "' AND PRODUCTID = '" & OriginalProductId & "'", SlxApplication.ConnectionString))
                Price = CDbl(GetField("IsNull(LISTPRICE, 0)", "STOCKCARDITEMS", "ACCOUNTID = '" & AccountId & "' AND PRODUCTID = '" & OriginalProductId & "'", SlxApplication.ConnectionString))
            Else
                ' ==============================================================
                ' The Item is not In the Stock Card so Need to Add it Manually
                '===============================================================
                ...
                Dim CURRENCYCODE
                CURRENCYCODE = GetField("Isnull(CURRENCYCODE,'CAD')", "ACCOUNT", "ACCOUNTID='" & AccountId & "'", SlxApplication.ConnectionString)
                Dim ItemKey
                ItemKey = GetField("MASITEMKEY", "PRODUCT", "ProductID='" & OriginalProductId & "'", SlxApplication.ConnectionString)
                ...
                CustKey = GetField("MASCUSTKEY", "ACCOUNT", " ACCOUNTID='" & AccountId & "'", SlxApplication.ConnectionString)
    ...
    Function GetListPrice(ByVal ItemKey, ByVal CurrencyId, ByVal CustKey)
        ...
        PriceSheetToUse = GetField("PRICEBASE", "tarCustAddr", " CustKey = '" & CustKey & "' order by UpdateDate desc", Me.SlxApplication.ConnectionString)
        '=========================================================================
    ...
    Function GetMargin(ByVal Productid, ByVal Accountid)
        ...
        ProducPriceGroupKey = GetField("MASPRODPRICEGROUPKEY", "PRODUCT", "PRODUCTID='" & Productid & "'", SlxApplication.ConnectionString)

        Dim TIMPRODPRICEGROUPID
        TIMPRODPRICEGROUPID = GetField("TIMPRODPRICEGROUPID", "TIMPRODPRICEGROUP", "CompanyID = '" & _CompanyId & "' and ProdPriceGroupKey  = '" & ProducPriceGroupKey & "'", Me.SlxApplication.ConnectionString)
    ...
    Function GetDefaultMargin(TIMPRODCATEGORYID, Accountid)
        ...
        NationalAcctCustId = CheckNull(GetField("MASNATIONALACCTID", "ACCOUNT", "ACCOUNTID='" & Accountid & "'", Me.SlxApplication.ConnectionString), "")
        ...
        ProdCategoryID = GetField("Isnull(ProdPriceGroupID,'')", "TIMPRODPRICEGROUP", "TIMPRODPRICEGROUPID='" & TIMPRODCATEGORYID & "'", Me.SlxApplication.ConnectionString)
    ...
    Function GetOriginalProduct(ByVal ActualId, ByVal CompanyId, ByVal IsInternalAccount, ByVal WarehouseId)
        ...
        If (CompanyId = "EBU" Or IsInternalAccount = True) Then
            ...
            If Strings.Right(ActualId, 1) = "4" Then
                ...
                productId = GetField("IsNull(ProductID, '')", "Product", filter, SlxApplication.ConnectionString)
                ...
                If productId <> "" Then
                    ...
                    'Regular Test
                    Dim ItemKey
                    ItemKey = GetField("MASITEMKEY", "PRODUCT", "PRODUCTID='" & productId & "'", SlxApplication.ConnectionString)
    ...
    Sub CreateRunTimeDatagrid()
        ...
        Dim conn As New OleDbConnection(Me.SlxApplication.ConnectionString)
        ...
        Try
            ...
            With objRS
                ...
                If Not (.BOF And .EOF) Then ' Check not at end/beginning
                    ...
                    Else
                        NextLineNumber = CInt(GetField(" ISNULL(MAX(LINENUMBER), 0)", "SALESORDERITEMS", "SALESORDERID='" & _SalesOrderId & "'", SlxApplication.ConnectionString)) + 1
                    ...
                    .Fields("CURRENTYEARSALESQTY").Value = GetField("IsNull(Sum(SOI.QUANTITY), 0) As Quantity",
                            ...
                            " And SO.ACCOUNTID = '" & _AccountId & "'", Me.SlxApplication.ConnectionString)
                    ...
                    .Fields("PREVIOUSYEARSALESQTY").Value = GetField("IsNull(Sum(SOI.QUANTITY), 0) As Quantity",
                            ...
                            " And SOI.QUANTITY <> 0" &
                            " And SOI.ACTUALID = '" & txtSKU.Text & "'" &
                            " And SO.ACCOUNTID = '" & _AccountId & "'", SlxApplication.ConnectionString)
                ...
                Else
                    ...
                    .Fields("UNIT").Value = GetField("UNIT", "PRODUCT", "PRODUCTID='" & _ProductId & "'", SlxApplication.ConnectionString)
                    ...
                    '.Fields("SLXLOCATIONID").Value =""
                    .Fields("UNITOFMEASUREID").Value = GetField("UNITOFMEASUREID", "PRODUCT", "PRODUCTID='" & _ProductId & "'", SlxApplication.ConnectionString)
                    '.Fields("PRICEDETAILDESCRIPTION").Value =""
                    '.Fields("PRICEDETAILNOTES").Value =""
                    '.Fields("ITEMLOCKED").Value =""
                    '.Fields("TICK").Value =""
                    ...
                    .Fields("MAX_STOCKLEVEL").Value = GetField("MAXSTOCKLEVEL", "PRODUCT", "PRODUCTID='" & _ProductId & "'", SlxApplication.ConnectionString)
                    .Fields("ORIGPRODUCTPRICE").Value = _OriginalProductPrice
                    .Fields("ORIGPRODUCTDISCOUNT").Value = _OriginalMargin
    ...
    Function GetProductCategory(ByVal Productid, ByVal Accountid)
        ...
        ProducPriceGroupKey = GetField("MASPRODPRICEGROUPKEY", "PRODUCT", "PRODUCTID='" & _ProductId & "'", SlxApplication.ConnectionString)

        Dim CategoryID
        CategoryID = GetField("TIMPRODPRICEGROUPID", "TIMPRODPRICEGROUP", "CompanyID = '" & _CompanyId & "' and ProdPriceGroupKey  = '" & ProducPriceGroupKey & "'", SlxApplication.ConnectionString)
        ...
        DistributorAccountid = GetField("DISTRIBUTORACCOUNTID", "USERSECURITY", "USERID='" & _AccountManagerId & "'", SlxApplication.ConnectionString)
    ...
    Sub ClearForm()
        Dim OrderType
        OrderType = GetField("ORDERTYPE", "SALESORDER", " SALESORDERID='" & _SalesOrderId & "'", SlxApplication.ConnectionString)

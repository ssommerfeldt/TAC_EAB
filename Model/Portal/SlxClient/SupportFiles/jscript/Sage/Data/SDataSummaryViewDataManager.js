                    ...
                    if (currentContext.duplicates) {
                        try {
                            if (!currentContext.applied) {
                                ...
                                var j;
                                for (j = 0; j < currentContext.duplicates.length; j++) {
                                    try {

                                        if (!currentContext.duplicates[j].context.applied) {
                                            currentContext.duplicates[j].context.applied = true;
                                            currentContext.duplicates[j].context.set('entity', entity);
                                        }
                                    }
                                    catch (err) {
                                        console.error(err);

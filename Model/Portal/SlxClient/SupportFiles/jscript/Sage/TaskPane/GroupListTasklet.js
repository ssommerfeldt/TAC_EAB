    ...
    var groupListTasklet = declare('Sage.Groups.GroupListTasklet', [_Widget, _LocalStorageMixin], {
        ...
        _groupChangedAndLoaded: function () {
            ...
            if (!(this._getGroupID() === 'LOOKUPRESULTS' && this.mode === 'Extended')) {
                ...
                //if we're not going to load the first row in the group, then reload the current entity because it's possible 
                //to click on the LookupResults group and then click a different entity (in the old group) while the LookupResults group 
                //is still in the process of loading - and that causes a problem where the current entity is not the one displayed in the ui
                this._reloadCurrentEntity();
            }
        ...
        refreshGrid: function () {
            ...
            // Make sure the entity is in this group
            var cec,
                curEntityId,
                selEntity,
                selEntityId,
                doSelect = true;
            if (this.grid.rowCount > 0) {
              selEntity = this.grid.getItem(position);
              if (selEntity)
                selEntityId = selEntity[this.keyAlias];
            }
            if (Sage.Services.hasService("ClientEntityContext")) {
                cec = Sage.Services.getService("ClientEntityContext");
                curEntityId = cec.getContext().EntityId;
                doSelect = Boolean(selEntityId && curEntityId && selEntityId === curEntityId);
            }
            if (doSelect) {
              this.selectGridRow(position);
            } else {
              // Remove the selection as the Entities don't match
            ...
            if (Sage.Services.hasService("ClientEntityContext")) {
                ...
                if (selected) {
					var service = Sage.Services.getService('IntegrationContractService');
                            if (service.isCPQIntegrationEnabled && this.context.CurrentEntity.toUpperCase() == 'QUOTE')
                                Sage.Utility.showDetailForm(this.context.CurrentEntity,selected[this.keyAlias]);                            
							else							
        ...
        _reloadCurrentEntity: function () {
            var contextservice = Sage.Services.getService("ClientContextService");
            var entityDisplayMode = contextservice.containsKey("modeid") ? contextservice.getValue("modeid") : "";

            if (entityDisplayMode.toLowerCase() === "detail") {
                var currentEntityId = contextservice.containsKey("ClientEntityId") ? contextservice.getValue("ClientEntityId") : "";
                var previousEntityId = contextservice.containsKey("PreviousEntityId") ? contextservice.getValue("PreviousEntityId") : "";

                if (Sage.Services.hasService("ClientEntityContext")) {
                    var clientEntityContext = Sage.Services.getService("ClientEntityContext");
                    clientEntityContext.navigateSLXGroupEntity(currentEntityId, previousEntityId, null);
                }
                //If we are currently displaying the extended group list, and the option to
                //hide extended group list on selection is enabled, we switch to standard view.
                if (this.hideExtendedGroupListOnSelection && (this.mode === 'Extended') && !this.sorting && !this.changingGroup) {
                    this.toggleMode('Standard');
                }
                //If the onselected was fired due to the grid being sorted, we clear the flag
                this.sorting = false;
            }
        },
                ...
                if (value !== null && typeof value !== 'undefined') {
                    ...
                    svc.get('FieldCount', 'ExtendedGroupList', lang.hitch(this, function (option) {
                        if (option && option.value) {
                            this.fieldCount = parseInt(option.value, 10);
        ...
        */
        _getHeight: function () {
            var commonTasksHeight = dojo.marginBox(dojo.byId("item_CommonTasksTasklet")).h;
        ...
        */
        _resizeTaskPane: function () {
            ...
            if (this.mode === 'Extended') {
                dijit.byId('item_CommonTasksTasklet').set('open', false);
            }
            else {
                dijit.byId('item_CommonTasksTasklet').set('open', true);
            }
        },
        _connectToCommonTasksToggleEvent: function () {
            var groupList = this;
            var commonTasksTasklet = dijit.byId('item_CommonTasksTasklet');

    ...
    var DesignSurface = declare('Sage.QuickForms.Design.DesignSurface', [_LayoutWidget, _EventMapMixin], {
                    ...
                    if (!selected)
                    {
                        array.forEach(this.selections, function(selection) {
                            if (selection.focusNode) {
                                domAttr.set(selection.focusNode, 'tabIndex', '-1');
                            }
                        });
        ...
        _clearSelectionDom: function() {
            if (this.selections.length > 0)
            {
                array.forEach(this.selections, function(selection) {
                    ...
                    if (selection.styleNode) {
                        domClass.remove(selection.styleNode, 'design-surface-cell-selected');
                    }
                });
            }
        },
        ...
        _syncNonVisibleDesigners: function(layout) {
            ...
            if (this.form.get('rowCount') > layoutRowCount)
            {
                ...
                rows = this.form.get('rows');

                for (i = (rows.length - 1); i > (layoutRowCount - 1); i--)
                {
                    if (rows[i].added)
            ...
            else if (this.form.get('rowCount') < layoutRowCount)
            {
                ...
                for (i = 0; i < (layoutRowCount - count); i++)
                {
            ...
            if (this.form.get('columnCount') > layoutColumnCount)
            {
                ...
                columns = this.form.get('columns');

                for (i = (columns.length - 1); i > (layoutColumnCount - 1); i--)
                {
                    if (columns[i].added)
            ...
            else if (this.form.get('columnCount') < layoutColumnCount)
            {
                ...
                for (i = 0; i < (layoutColumnCount - count); i++)
                {
        ...
        _buildConnectionTree: function(layout, parent, nodesp, rootp) {

...
/// </summary>
public partial class SmartParts_ProximitySearch_ContactDistanceSearch :  UserControl, ISmartPartInfoProvider
{
    ...
    private void CheckForDialogParameters()
    {
        if (DialogService != null)
        {
            ...
            if (o != null)
            {
                ...
                // Get the entity
                var entity = EntityFactory.GetById<IContact>(entityId);
    ...
    private void InitializePickListItems()
    {
        ...
        if (string.IsNullOrEmpty(pklType.PickListName))
            pklType.PickListName = "Contact Type";
    }
        ...
        if (cbxLocation.Items.Count == 0)
        {
            ...
            if (Request.QueryString["start"] != null)
            {
                try
                {
                    ...
                    var contact = EntityFactory.GetById<IContact>(entityId);
                    if (contact != null && contact.Address != null && contact.Address.IsGeocoded())
                    {
                        cbxLocation.Items.Add(new ListItem(contact.Name, contact.Address.Id.ToString()));
                        startIndex = 1;
                    }
                }
                catch { }
    ...
    protected void btnCreateGroup_OnClick(object sender, EventArgs e)
    {
        if (ExecDistanceSearch() > 0)
        {
            if (!string.IsNullOrEmpty(createGroupName.Value))
            {
                ...
                var groupId = DistanceSearchHelper.CreateContactDistanceGroup(groupName,
                    lat, lon, Convert.ToDouble(cbxDistance.Text),
                    pklType.PickListValue, pklStatus.PickListValue,
    ...
    protected void btnDisplayMap_OnClick(object sender, EventArgs e)
    {
        if (ExecDistanceSearch() > 0)
        {
            ...
            ScriptManager.RegisterClientScriptBlock(this, GetType(), Guid.NewGuid().ToString(),
                string.Format(
                "if(!window.open('" + Page.ResolveClientUrl("~/SmartParts/ProximitySearch/PxMap.aspx") + "?from={0}&fromlat={1}&fromlon={2}&type={3}&dist={4}&ctype={5}&cstat={6}&sort={7}&asc={8}&cnt={9}&lcid={10}', '_blank')){{ alert('" + GetLocalResourceObject("Error_UnableToOpenMapWindow").ToString() + "'); }}",
                HttpUtility.UrlEncode(txtCustom.Text), lat, lon,
                HttpUtility.UrlEncode("IContact"),
                HttpUtility.UrlEncode(cbxDistance.Text),
                HttpUtility.UrlEncode(pklType.PickListValue),
                HttpUtility.UrlEncode(pklStatus.PickListValue),
    ...
    public int ExecDistanceSearch()
    {
        ...
        //We use the alias ContactName to remain consistent with App Architect syntax. 
        double lat, lon;
        GetLatLonForLocation(out lat, out lon);

        var results = DistanceSearchHelper.ContactDistanceSearch(lat, lon, Convert.ToDouble(cbxDistance.Text),
            pklType.PickListValue, pklStatus.PickListValue,
    ...
    public ISmartPartInfo GetSmartPartInfo(Type smartPartInfoType)
    {
        ...
        foreach (Control c in ContactDistanceSearch_RTools.Controls)
        {

...
function (AccordionContainer, _LayoutWidget, NavBarPane,
    ...
    var navBar = declare('Sage.UI.NavBar', [AccordionContainer, _SplitterEnhancedMixin], {
        ...
        startup: function () {
            ...
            if (parseInt(cookie(that.navPin + '_width'), 10) > 0) {
                domStyle.set(navContainer, 'width', cookie(that.navPin + '_width'));
            }

            // Pin navigation to page on click. Set cookie to keep nav open 
            // for next page load.
            ...
            // In case nav pane is unpinned and open - close it
        ...
        layout: function () {
            ...
            array.forEach(this.getChildren(), function (child) {
                if (child != openPane) {
                    var childHeight = parseInt(domStyle.getComputedStyle(child.getParent().domNode.children[0]).height, 10),
                     childMarginBottom = parseInt(domStyle.getComputedStyle(child.getParent().domNode).marginBottom, 10),
                     childMarginTop = parseInt(domStyle.getComputedStyle(child.getParent().domNode).marginTop, 10),
                     childBorderTop = parseInt(domStyle.getComputedStyle(child.getParent().domNode).borderTopWidth, 10),
                     childBorderBottom = parseInt(domStyle.getComputedStyle(child.getParent().domNode).borderBottomWidth, 10);
            ...
            logoHeight = me.b + me.t + parseInt(cs.height, 10) + parseInt(cs.paddingTop, 10) + parseInt(cs.paddingBottom, 10) + be.h;
            ...
            // Vertically position Nav pane to accommodate logo

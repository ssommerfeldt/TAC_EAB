...
define("Sage/UI/Controls/GridParts/Columns/SlxLink", [
    'Sage/Format',
    'Sage/Utility',
    'dojo/_base/declare',
    'dojo/_base/lang',
...
function (Format, Utility, declare, lang, dString) {
    Sage.Region = {
        decimalSeparator: '.',
    ...
    var widget = declare("Sage.UI.Controls.GridParts.Columns.SlxLink", null, {
        constructor: function (args) {
            lang.mixin(this, args);
        },
        _dataTypeChecked: null,
        _layout: null,
        _getDisplayValue: function (value, inItem, dispVal) {
            var sValue = dispVal || "";
            if (this._layout === null) {
                    var sTemp = dispVal ? dispVal : (this.value || this.defaultValue);
            ...
            if (this._layout.fieldType === "DateTime" && this._layout.format === "DateTime") {
                var sDate = value ? value : (this.value || this.defaultValue);
        ...
        format: function (value, inItem) {
            ...
            if (this.editable && (this.alwaysEditing || (i.rowIndex == value && i.cell == this))) {
                return this.formatEditing(d, value);
            } else {
                dispVal = d;
                ...
                if (this.appliedSecurity && this.appliedSecurity !== '') {
                    var svc = Sage.Services.getService("RoleSecurityService");
                    if (svc) {
                        if (!svc.hasAccess(this.appliedSecurity)) {
                            dispVal = this._getDisplayValue(value, inItem, dispVal);
                            return dispVal;
                        }
                    }
                }
                if (this.idField && this.idField !== '') {
                    //entity page linking...
                    ...
                    if (entKey) {
                        dispVal = Utility.htmlEncode(this._getDisplayValue(value, inItem, dispVal));
                        if (pagename.toUpperCase() === 'HISTORY') {
                            ...
                            if (pagename.toUpperCase() == 'QUOTE' && Utility.isEQEnabled())
                            {
                              return dString.substitute('<span onclick="Sage.Utility.showDetailForm(\'${0}\',\'${1}\')" class="activity-type-link">${2}</span>', [pagename,entKey,dispVal]);
                            }
                            else{
                                return dString.substitute('<a href="${0}.aspx?entityid=${1}&modeid=Detail${2}"${3} id="${1}" title="${4}">${4}</a>', [pagename, entKey, moreQuerystring, target, dispVal]);
                            }
                        }
                    }
                } else {

...
define("Sage/MainView/GroupMgr/GroupsSDataDetailViewDataManager", [
    ...
    function(sDataServiceRegistry, SDataStore, declare, lang) {
        var groupsSDataDetailRequestQueue = declare('Sage.MainView.GroupMgr.GroupsSDataDetailRequestQueue', null, {
            select: [],
            resourceKind: '',
            fetching: false,
            include: [],
            ...
            constructor: function (options) {
                ...
                this.requestHash = {};
                this.fetching = false;
                var serviceName = typeof options !== 'undefined' && options !== null ? options.serviceName || 'system' : 'system';
                this.service = Sage.Data.SDataServiceRegistry.getSDataService(serviceName);
            },
        ...
        var groupsSDataSummaryViewDataManager = declare('Sage.MainView.GroupMgr.GroupsSDataDetailViewDataManager', null, {
            keyField: "$key",
            constructor: function () {
                this.queue = new groupsSDataDetailRequestQueue();
                this.requestTimeout = false;
            },
            requestDataNoWait: function(entityid, widget, requestConfiguration) {
                var tempQueue = new groupsSDataDetailRequestQueue(requestConfiguration);
            ...
            receiveData: function(context, data) {
                ...
                if (context.resourceKind === "groupReleasesView") {
                    var optionsSvc = Sage.Services.getService('UserOptions');
                    if (optionsSvc) {
                        optionsSvc.getByCategory('MainView', function (options) {
                            var i, option, data = options['$resources'];
                            for (i = 0; i < data.length; i++) {
                                option = data[i];
                                if (option.name === 'HiddenGroups') {
                                    entity.IsHidden = option.value.indexOf(id) >= 0 || option.value.indexOf(entity.BasedOn) >= 0;
                                    break;
                                }
                            }
                        }, this);
                ...
                if (currentContext && lang.isArray(currentContext._attachPoints) && currentContext._attachPoints.length > 0) {
                    try {
        ...
        return groupsSDataSummaryViewDataManager;
    });
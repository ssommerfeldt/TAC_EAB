...
function (
    ...
    var groupLookup = declare('Sage.Groups.GroupLookup', null, {
        ...
        _showInitializedLookup: function () {
            ...
            entityReq.setResourceKind('entities(' + "'" + tableNameFromLayout + "'" + ')');
            ...
            entityReq.read({
                success: function (data) {
                    if (typeof (data) !== 'undefined' && typeof (data.$resources) !== 'undefined' && data.$resources.length == 1) {
                        ...
                        if (!self._dlgWindow) {
                            self._dlgWindow = new lookupDialog({ conditionMgr: self._conditionMgr });
                        }
                        if (!self._dlgWindow.conditionMgr) {
                            self._dlgWindow.set('conditionMgr', self._conditionMgr);
                        }
                        self._dlgWindow.set('title', self.displayName);
                        self._dlgWindow.show();
                    }
                },
        ...
        doLookup: function (args) {
            ...
            var coditionsString = this.getConditionsString();
                ...
                success: function (response) {
                    ...
                    if (context.CurrentFamily && this._currentLayoutFamily && context.CurrentFamily.toUpperCase() === this._currentLayoutFamily.toUpperCase()) {
                        aspect.after(groupContextService, 'onCurrentGroupChanged', lang.hitch(this, function () {
                          ...
                          topic.publish("/group/lookup/success", { 'conditions': coditionsString });
                        ...
                        groupContextService.setCurrentGroup('LOOKUPRESULTS', '', coditionsString);
